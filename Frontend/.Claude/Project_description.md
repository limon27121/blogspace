Develop a complete frontend Blog Management Application using Next.js.
...
Created date
If the blog does not exist, show an appropriate:
...
userId
The backend determines the user from the authentication token.
...
/dashboard/blogs
Display blogs in either:

Cards
Table
Recommended table:

TitleCategoryAuthorCreatedActions
Playwright Basics	Testing	John	Sep 2	Edit / Delete
18. Update Blog
Clicking Edit should navigate to:
...
Before deletion, show confirmation such as:
...
Users must not be given frontend controls for changing:
...
Update the fixed Navbar avatar immediately.
Do not require the user to logout/login again.
...
Loading blogs...
or loading skeletons/spinners.

Buttons should also indicate pending state.
...
A complete application should support the following journey:
...
Properly structured Next.js project
.gitignore
.env.example
README
Screenshots of major application pages
